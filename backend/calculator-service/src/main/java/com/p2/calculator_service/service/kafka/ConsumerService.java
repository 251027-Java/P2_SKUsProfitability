...
public class ConsumerService {
    ...
    public void listen(String message) { // Receive as String
        try {
            // 1. Manually convert the JSON string to a Product object
            ...
            // 2. Save/Update the local database
            productRepository.save(productFromKafka);

            System.out.println("Successfully saved local copy: " + productFromKafka.getSku());

        } catch (Exception e) {
            // This will catch errors if the JSON doesn't match your Product class
            System.err.println("Failed to parse or save product: " + e.getMessage());
        }
    }

//    @KafkaListener(topics = "sku-updates", groupId = "calculator-group")
//    public void listen(Product productFromKafka) { // Note: Receiving Product object
//        try {
//            System.out.println("Received sync for SKU: " + productFromKafka.getSku());
//
//            // 1. UPSERT (Update or Insert) the product into the local database
//            // This is how the 'products' table in the Calculator DB gets populated!
//            productRepository.save(productFromKafka);
//
//            System.out.println("Successfully saved/updated local copy of product: " + productFromKafka.getSku());
//
//        } catch (Exception e) {
